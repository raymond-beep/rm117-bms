...
**Last updated:** 2026-07-01 (late) — Financial tab **"Sent" income basis** (default) + **contract-at-invoicing** (signed-proposal viewer for existing jobs, app-proposal fee reference for new ones) + **Financial on the mobile tab bar**. Ray reviewing before telling Angelena to use it → next session likely = his refinements.
...
- **▶ NEXT SESSION likely = Ray's refinements from using it (before he hands it to Angelena).** He's reviewing the Sent basis + contract viewer + mobile layout live and may want polish. Ask what he found.
