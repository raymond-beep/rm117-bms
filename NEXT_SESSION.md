...
**Last updated:** 2026-07-05 (eve) — Session did three things: 🔐 **rotated the exposed `95YW…` Intuit secret**, 🏷️ **renamed Drawing QA → "Checksets" + reordered the sidebar**, and 🔍 **ran a full staff UX audit → 18 ID'd findings in `USER_TEST_FINDINGS.md` (Round 2)**. **▶ NEXT SESSION = start the UX-audit backlog** (Ray to prioritize; suggested first: **`UX2-01`** proposal file-noise + **`UX2-17`** dead global search), and/or triage Angelena's real feedback (still pending — ask Ray). **Loose ends:** add `QBO_CLIENT_SECRET` to Vercel _Preview_ (Ray, dashboard); the Dunn `24_008` pair.
