...
**Last updated:** 2026-07-01 (late) — Financial tab **"Sent" income basis** (default) + **contract-at-invoicing** (signed-proposal viewer for existing jobs, app-proposal fee reference for new ones) + **Financial on the mobile tab bar** + **Financial-tab caching** (instant repeat loads). Ray reviewing before telling Angelena to use it. **NEXT = code-cleanup pass** on today's changes (Ray's request) + Angelena's layout feedback.
