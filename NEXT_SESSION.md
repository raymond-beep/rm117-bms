...
**Last updated:** 2026-07-01 (evening) — Financial tab reworked around Angelena's feedback: **"Sent" income basis** (invoices billed for completed work) is now the default; verified vs her books.
...
- **▶ PARKED FOLLOW-UP (Ray's next build, explicitly deferred this session): proposal fee schedule → invoicing.** When invoicing a job, surface the **contract amount from that job's saved proposal** so staff know what the next milestone payment should be per the signed proposal. Fits the "Send to QuickBooks" panel (`src/components/job-editor/QboInvoicePanel.jsx`) — proposals already store per-phase fee schedules in `proposals.content` jsonb (`api/proposals.js`). Open question for Ray: **auto-fill the invoice amount from the proposal, or just display the contract figure alongside** the manual entry? (Ray leaned toward showing it; confirm.)
