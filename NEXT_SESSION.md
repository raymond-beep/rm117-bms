...
**Last updated:** 2026-07-01 (late) — Financial tab **"Sent" income basis** (default) + **contract-at-invoicing** (signed-proposal viewer for existing jobs, app-proposal fee reference for new ones) + **Financial on the mobile tab bar** + **Financial-tab caching** (instant repeat loads). Ray reviewing before telling Angelena to use it.

> ## 🧹 DO THIS FIRST NEXT SESSION — code-cleanup pass, *before* any revisions
> **Ray's explicit ask (2026-07-01):** run a **`/simplify`-style cleanup** on today's changes **before** touching any of Angelena's revisions. Today added ~665 lines across `Financial.jsx`, `api/qbo/financials.js`, `api/_lib/qbo.js`, `api/_lib/qbo-reports.js`, `QboInvoicePanel.jsx`, `ProposalDocs.jsx`, `api/jobs/proposal-docs.js`, `PaymentsTab.jsx`, `api/proposals.js`, `styles.css`. The pass was deferred so we wouldn't destabilize the version Angelena is reviewing — do it first thing, get tests+build green, **then** apply her feedback. Candidate cleanups: dedupe the two stat-tile JSX branches in `Financial.jsx`, the sent-mode P&L/quarter branching in `financials.js`, and the duplicated `round2` (financials.js vs qbo-reports.js); confirm no unused exports.
