...
const AUTOMATED_LOCALPART = new Set([
  ...
  // The app's OWN portal sender. Sign-in codes land in Ray's inbox because he is
  // also a test client; the BMS announcing itself is not correspondence.
  'portal',
]);
...
// ⭐ THE RELIABLE BULK SIGNAL IS A HEADER, NOT A DOMAIN LIST.
//
// SAAS_DOMAINS can only ever catch senders someone thought to enumerate, and the
// firm's real inbox is full of ones nobody would: zillow.com, autodesk.com,
// dealmachine.com, plus coaching/newsletter mail from addresses that look exactly
// like a person ("AI with Mariah", "Elise Knaack", "Ray Fu"). Measured on Ray's
// mailbox, roughly 35 of 60 threads in the "Work" scope were bulk marketing —
// so the scope was mostly noise and the feature's core promise, that you can
// watch this page instead of Gmail, did not hold.
//
// Every legitimate bulk sender sets List-Unsubscribe (RFC 2369/8058) — it is
// required to reach an inbox at scale, so marketers are motivated to include it,
// while a township clerk, a structural engineer or a client writing by hand never
// does. Precedence: bulk and Auto-Submitted cover automated mail that isn't a
// mailing list. This keys on the mail's own declaration of what it is.
//
// ⚠️ Deliberately does NOT look at the body or subject for marketing words —
// "last chance", "don't miss out". A permit expediter chasing a deadline writes
// like that too, and wrongly hiding one real email costs more than showing ten
// newsletters.
export function isBulkMail(headers = {}) {
  const h = headers || {};
  if (h['list-unsubscribe'] || h['list-id'] || h['list-post']) return true;

  const precedence = String(h.precedence || '').toLowerCase().trim();
  if (['bulk', 'junk', 'list'].includes(precedence)) return true;

  // RFC 3834: anything other than "no" means generated by a machine.
  const autoSubmitted = String(h['auto-submitted'] || '').toLowerCase().trim();
...
// `headers` is the lowercased header map for the message (see headerMap) and is
// optional — callers that don't have it get the previous behaviour.
export function classifySender(sender, matchResult, headers = {}) {
  const email = (sender?.email || '').toLowerCase().trim();
  const bulk = isBulkMail(headers);

  // An EXACT email match to a client (or one of their contacts) outranks
  // everything, including the bulk headers: if a client's own address sends
  // through something that stamps List-Unsubscribe, they are still the client.
  if (matchResult?.isClient && matchResult.via === 'email') return 'client';

  // Bulk beats the SURNAME fallback, which is the guess. This is what stops
  // "AI with Mariah" or an "Elise Knaack" newsletter being promoted to a client
  // (or to `project`) because its display name happens to share a surname with
  // one of 162 jobs — the loose case the old classifier could not see.
  if (bulk) return 'noise';
  if (matchResult?.isClient) return 'client';

  // An automated ROLE address at our own domain is still automated — checked
  // before the staff-domain test so portal@rm117.com does not read as a
  // colleague. A named staffer (angelena@) is unaffected.
  const at = email.indexOf('@');
  const localPart = at > 0 ? email.slice(0, at).replace(/\+.*$/, '') : '';
  if (email.endsWith(STAFF_DOMAIN)) {
    return AUTOMATED_LOCALPART.has(localPart) ? 'noise' : 'staff';
  }
