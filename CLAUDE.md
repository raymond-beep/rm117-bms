...
| **Weekly Planner** (`/delegation`) | Sidebar label "Weekly Planner"; route stays `/delegation`. Digital weekly delegation sheet (replaces Ang's paper grid): Mon–Fri × employee grid, one board per week (Monday-keyed). **Each cell is a CHECKLIST** — type an item, Enter to add another, tick it off (checked box + line-through, plus a "3 of 7 done" line). UI `src/components/delegation/Delegation.jsx`; API `api/delegation.js` (GET/POST/PATCH/DELETE, staff-gated); tables `delegation_members` + **`delegation_tasks`** (migration 0022). ⚠️ **THE PEN WAS REMOVED 2026-07-31** — ink was Ang's request and she never used it (`delegation_strokes` = **2 rows** in the board's life, vs 31 typed notes all hers, already written one item per line). Gone with it: the canvas, the Pen/Type toggle, colour swatches, board zoom (it existed only to make cells big enough to HAND-WRITE in) and the ink-sync cooldown. `delegation_strokes` + `delegation_notes` are **kept, unread** — notes are what 0022 backfilled the 68 tasks FROM, which is what makes the conversion reversible. **Row-level write perms enforced server-side** (own row, or admin=Ang) via `canWrite()`/`canModifyTask()` (unit-tested) — not RLS. ⚠️ `canModifyTask` keys on the ROW, not the author: Ang assigns most items, so an author-based rule would stop people ticking off their own work. Live sync = 4s polling. Roster (`delegation_members`, live DB is truth): Tom·Ray·Nicole·Ang·Dani. Shared **"Everyone" lane** pinned at the top (reserved `row_owner_email = '__studio__'`, admin-write only) for firm-wide items. **⭐ The `__studio__` sentinel lives in 3 files that must stay in sync: `Delegation.jsx`, `api/delegation.js`, and `MyWeekWidget.jsx`.** The dashboard **"My week" widget** (`MyWeekWidget.jsx`, on Home) shows the signed-in user's own row **and** the Everyone lane, read-only with an open/done count — ticking still happens in the planner. |
...
`delegation_members` + `delegation_tasks` (Weekly Planner; migration `0022`)
